...
wapplyp <- function(x, ws=window, by = NULL, FUN = NULL, cores=cores, ...) {
	FUN <- match.fun(FUN)
	if (is.null(by)) by <- ws
	lenX <- length(x)
	# get indices
	...
	# Parallelised lapply 
	OUT <- mclapply(SEQ2, function(a) FUN(x[a], ...),mc.cores=cores)
...
compute_stats  <- function(mydf=mydf,background=background,window=window,by=by,width=width,cores=cores) {
	...
	manwh_rollmedianlogr <- wapplyp(indices,ws=window,FUN=distance_weighted_man_u,cores=cores,column="logrdev",cases=mydf,background=background,by=by)

	say("finished first wp, starting second")

	manwh_bdev           <- wapplyp(indices,ws=window,FUN=distance_weighted_man_u,cores=cores,column="bdev",   cases=mydf,background=background,by=by)
	
	say("finished second wp, starting manwh")

	mydf[is,"manwh_combined"] <- fisher.method(data.frame(manwh_bdev,manwh_rollmedianlogr),mc.cores=cores)$S
...
mrmosaic_calling_func <- function(path=path,t=t,cores=cores,window=40,width=5,by=1) {
    #Get the name of the sim file out of the path and use it for parsing out the coordinates
    pid <- tail(unlist(strsplit(path, "[.]")),1)
    ...
    for (chr in CHROMOSOMES) {
    	...
    	chrsubset <- compute_stats(mydf=chrsubset,background=background,window=window,cores=cores,by=by,width=width)
...
annot_ll_and_recalc_sig <- function (results) {
	load("/lustre/scratch113/projects/ddd/users/dk6/projects/mosaicism/exome/simulations/simulate_events/expt15.analysis/mrm_annot/gada10/ecdf_GADAamp.merged.gada.10.R3.Robj")
	load("/lustre/scratch113/projects/ddd/users/dk6/projects/mosaicism/exome/simulations/simulate_events/expt15.analysis/mrm_annot/gada10/ecdf_nprobes.merged.gada.10.R3.Robj")
...
in_path_file <- args[1]
out_dir      <- args[2]
if (is.na(args[3])) { out_dir <- paste(head(strsplit(in_path_file, "/")[[1]],n=-1),collapse="/") }

# Prepare names
in_file         <- tail(strsplit(in_path_file, "/")[[1]],n=1)
in_file_prefix  <- gsub(x=in_file,pattern=".extraction.txt",replacement="") # pid.sid
out_file_prefix <- in_file_prefix
opf1            <- paste(out_dir, paste(out_file_prefix, "mrm.txt",sep="."), sep="/")

# Prepare first output file
results         <- mrmosaic_calling_func(in_path_file,t=10)
results         <- annot_ll_and_recalc_sig(results)
...
# Plotting
plot_lattice(df_all, df_sig, in_file_prefix, out_dir) 
for (chr2plot in unique(df_sig$Chrom)) plot_single_chr_of_interest(df_all,df_sig,in_file_prefix,out_dir,chr2plot,ptsize=0.5)
